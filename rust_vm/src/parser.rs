...
impl Parser {
    ...
    /// 检查当前token的值,然后推进
    pub fn match_token_value(&mut self, value_arr: &[&str]) -> bool {
    ...
    /// 检查当前token的值,不推进
    pub fn check_token_value(&self, value_arr: &[&str]) -> bool {
    ...
    pub fn primary_expr(&mut self) -> Expr {
        if self.match_token_type(TokenType::数字字面量) || self.match_token_type(TokenType::布尔值)
        {
            let Token {
                value,
                column,
